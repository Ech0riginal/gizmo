...
impl V3 {
    fn core_deserializer<'a>(blob: Type<'a>) -> Result<GValue, Error> {
        match blob.tag {
            ...
            // Tag::TinkerGraph => blob.value.deserialize::<Self, TinkerGraph>().map(GValue::from),
            // Tag::Tree => blob.value.deserialize::<Self, Tree>().map(GValue::from),
            // Tag::Vertex => blob.value.deserialize::<Self, Vertex>().map(GValue::from),
            // Tag::VertexProperty => blob.value.deserialize::<Self, VertexProperty>().map(GValue::from),
            ...
            // Tag::TraversalMetrics => blob.value.deserialize::<Self, TraversalMetrics>().map(GValue::from),
